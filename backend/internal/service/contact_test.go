...
func TestContactService_UpdateContactFromEmail(t *testing.T) {
	...
	err = svc.UpdateContactFromEmail("new@example.com", "New User", now)
	if err != nil {
		t.Fatalf("First update failed: %v", err)
	}

	var c model.Contact
	db.First(&c, "email = ?", "new@example.com")
	...
	err = svc.UpdateContactFromEmail("new@example.com", "Updated Name", now.Add(1*time.Hour))
	if err != nil {
		t.Fatalf("Second update failed: %v", err)
	}

	var c2 model.Contact
	db.First(&c2, "email = ?", "new@example.com")
