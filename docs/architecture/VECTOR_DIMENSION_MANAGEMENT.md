# EchoMind 向量维度管理指南 (DEPRECATED)

> **⚠️ DEPRECATED**: 本文档描述的动态向量维度管理策略已被废弃。从 v1.1.3 版本开始，系统标准化使用 **1536** 维度（兼容 OpenAI 标准），不再支持动态维度切换。

---

## 📋 概述

EchoMind采用**规约化向量维度管理策略**，系统在运行时只支持一种固定的向量维度。这种设计确保了系统的稳定性和性能，但要求在更换嵌入模型时执行必要的迁移步骤。

## ⚠️ 核心原则

### 1. 单一维度约束
- 系统数据库schema定义固定向量维度：`vector(1024)`
- 所有向量必须符合相同的维度要求
- 不同维度的向量无法在同一系统中共存

### 2. 模型与维度绑定
- 每个嵌入模型具有固定的原生维度
- 系统配置必须明确指定目标维度
- 配置与实际模型维度不匹配时会导致错误

### 3. 全量重建机制
- 更换向量维度必须重建所有现有向量
- 使用Reindex工具执行批量重建
- 重建期间系统搜索功能不可用

### 4. 数据一致性保证
- 数据库层面强制维度约束
- 应用层面进行基本验证
- 文档提供明确的操作指导

## 🔧 支持的嵌入模型

| Provider        | 模型名称               | 原生维度 | 推荐使用场景      | 成本效益 |
| --------------- | ---------------------- | -------- | ----------------- | -------- |
| **SiliconFlow** | BAAI/bge-m3            | **1024** | 中文优化/当前默认 | ⭐⭐⭐⭐⭐    |
| **OpenAI**      | text-embedding-3-small | 1536     | 高精度英文场景    | ⭐⭐⭐      |
| **Gemini**      | text-embedding-004     | 768      | Google生态集成    | ⭐⭐⭐⭐     |
| **Ollama**      | nomic-embed-text       | 768      | 本地部署方案      | ⭐⭐⭐      |

**注意**: 当前系统配置为1024维度（SiliconFlow），其他模型需要相应调整。

## 🔄 嵌入模型更换流程

### 准备阶段
```bash
# 1. 确认新模型的维度信息
# 2. 备份现有向量数据（重要！）
docker exec deploy-db-1 pg_dump -U user -d echomind_db -t email_embeddings > embeddings_backup_$(date +%Y%m%d_%H%M%S).sql

# 3. 停止应用服务
make stop-backend
```

### 执行阶段
```bash
# 4. 更新数据库schema（以1024维度为例）
docker exec deploy-db-1 psql -U user -d echomind_db -c "ALTER TABLE email_embeddings ALTER COLUMN vector TYPE vector(1024);"

# 5. 更新应用配置
# 编辑 configs/config.yaml:
# - ai.active_services.embedding: 新provider
# - ai.providers.newprovider.embedding_dimensions: 新维度值

# 6. 清理现有向量数据
docker exec deploy-db-1 psql -U user -d echomind_db -c "DELETE FROM email_embeddings;"
```

### 重建阶段
```bash
# 7. 重新生成所有向量
go run cmd/reindex/main.go --config configs/config.yaml

# 8. 验证重建结果
docker exec deploy-db-1 psql -U user -d echomind_db -c "SELECT dimensions, COUNT(*) FROM email_embeddings GROUP BY dimensions;"

# 9. 重启应用服务
make run-backend
```

### 验证阶段
```bash
# 10. 测试搜索功能
curl -X GET "http://localhost:8080/api/v1/search?q=test&limit=5" \
  -H "Authorization: Bearer YOUR_JWT_TOKEN"
```

## 🚨 常见错误与解决方案

### 错误1: 维度不匹配
```
ERROR: different vector dimensions 1536 and 1024 (SQLSTATE 22000)
```

**原因**: 数据库schema与向量实际维度不匹配
**解决**: 执行完整的模型更换流程，更新数据库schema

### 错误2: 空向量
```
failed to embed query: empty embedding vector generated
```

**原因**: AI提供商返回空向量或配置错误
**解决**: 检查API密钥、模型配置和网络连接

### 错误3: 重建失败
```
Reindex complete {"success": 0, "failed": 22}
```

**原因**: 配置错误、API限制或数据库连接问题
**解决**: 检查配置文件、API配额和数据库状态

## 📊 性能考虑

### 维度与性能关系
| 维度     | 内存消耗 | 搜索速度 | 存储空间 | 精度表现 |
| -------- | -------- | -------- | -------- | -------- |
| **768**  | 低       | 快       | 小       | 良好     |
| **1024** | 中等     | 中等     | 中等     | 优秀     |
| **1536** | 高       | 慢       | 大       | 最佳     |

### 推荐配置
- **生产环境**: 1024维度（平衡性能与精度）
- **开发环境**: 768维度（节省资源）
- **研究环境**: 1536维度（最高精度）

## 🛠️ 维护最佳实践

### 定期检查
```bash
# 检查向量维度一致性
docker exec deploy-db-1 psql -U user -d echomind_db -c "SELECT dimensions, COUNT(*) FROM email_embeddings GROUP BY dimensions;"

# 检查向量索引状态
docker exec deploy-db-1 psql -U user -d echomind_db -c "\di email_embeddings_vector_idx"
```

### 备份策略
- 每周自动备份向量数据
- 模型更换前手动备份
- 保留最近3个月的备份文件

### 监控指标
- 向量生成成功率
- 搜索响应时间
- 数据库连接状态
- API调用频率限制

## 📚 参考文档

- [API搜索时序图](./api_search_sequence_diagram.md)
- [系统架构文档](./ARCHITECTURE.md)
- [部署指南](./DEPLOYMENT.md)
- [故障排除手册](./TROUBLESHOOTING.md)

## 💡 技术支持

如遇到向量维度相关问题，请按以下步骤操作：

1. **检查配置**: 确认`configs/config.yaml`中的维度设置
2. **查看日志**: 检查应用和数据库日志中的错误信息
3. **参考文档**: 查阅本文档和相关技术文档
4. **联系支持**: 提供详细的错误信息和系统配置

---

**更新时间**: 2025-11-26
**版本**: v1.0
**维护者**: EchoMind开发团队