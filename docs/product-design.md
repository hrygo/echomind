...
*   **语音处理 (Voice)**：
    *   接收微信语音消息 -> 调用 STT (Speech-to-Text) 转文字。
    *   文本意图识别 -> 执行查询/记事 -> TTS (Text-to-Speech) 返回语音回复 (可选)。
*   **意图路由**：
    *   文本/语音 -> AI Pipeline (意图识别: 是查询邮件? 还是闲聊?)。
...
### 2.6 全局导航与布局 (Global Navigation & Layout)
*   **统一侧边栏 (Unified Sidebar)**:
    *   **顶部**: 品牌 Logo + 视图切换器 (Executive/Manager/Dealmaker)。
    *   **中部**: 核心功能导航 (Dashboard, Inbox, Contacts, Insights)。
    *   **底部**: **"个人中心与设置" (User & Settings)** 统一入口。
        *   点击头像弹出菜单：包含 [个人资料]、[账号设置]、[偏好设置]、[登出]。
        *   **设计意图**: 将低频但重要的设置功能收纳于底部，符合用户习惯，且保持界面整洁。
*   **设置中心布局 (Settings Center)**:
    *   采用双栏布局：左侧为设置分类 (Account, Integrations, Notifications, Billing)，右侧为具体配置项。
    *   **即时预览**: 修改设置（如主题色、通知偏好）时，右侧或全局实时反馈变化。

### 2.7 用户界面：双视图模式 (UI: Dual-View Modes)
