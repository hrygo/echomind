...
`SearchHandler`是搜索功能的入口点，负责接收HTTP请求、验证参数并调用搜索服务。
...
**节来源**
- [search.go](file://backend/internal/service/search.go#L55-L99)
...
**图表来源**
- [search.go](file://backend/internal/service/search.go#L62-L63)

## 前端交互

### CopilotInput组件

前端的`CopilotInput`组件提供了直观的搜索界面，支持智能切换搜索模式：
...
**图表来源**
- [CopilotInput.tsx](file://frontend/src/components/copilot/CopilotInput.tsx#L24-L74)

### API调用实现

前端通过标准的fetch API调用搜索服务：
...
**节来源**
- [CopilotInput.tsx](file://frontend/src/components/copilot/CopilotInput.tsx#L46-L69)
